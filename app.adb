...
procedure App is
    ...
    begin
        for I in 0..(N - 1)  loop               -- Setting the required Automatas.
            ...
            if I = (N - 1) / 2 then
                Temp_Automata.Bit := 1;
                Set_Pixel(Image_Width, 0, (I - 1), WHITE);    
            else
                Temp_Automata.Bit := 0;
            end if;
    ...
    procedure Reset_Sequence(N : in INTEGER) is
            ...
            if Temp_Automata.Previous = null or Temp_Automata.Next = null then
                ...
                if Last_Pattern.Bit = 1 then
                    if First_Pattern.Bit = 0 then
                        Temp_Automata.Bit := 0;    
                    elsif First_Pattern.Bit = 1 then       
                        Temp_Automata.Bit := 1; 
                    end if;
                end if;                          
            else
...
Main_Loop:
    for I in 1..(Image_Height - 1) loop
        if I + 1 = Image_Height then
            ...
            Put_PPM(File_Name, Image_Height, Image_Width);
            Put_Line("PPM generated successfully.");
            Reset_Image;
            Reset_Sequence(Image_Width);
            exit Main_Loop;
        else
            Update_Sequence(I, Image_Width, Color); 
        end if;
